...
-- File name: exit1_2.lua
-- Purpose: show the exit factoid slide for level 1, stage 2
-- Group 5:
--    Terry Liu
--    Raymond Ononiwu
--    William Thomas
...
print("Loading assets.exit.exit1_2...");
...
local function onSceneTouch( self, event )
  if event.phase == "began" and not kiosk.on then
    ...
    storyboard.gotoScene( "assets.exit.level1", "crossFade", 250 );
...
function scene:createScene( event )
  ...
  exit = display.newImage("images/exit/stage2complete.png", true);
...
function scene:willEnterScene( event )
  resetCamera();
  if(kiosk.labels["exiting_completion_s2"] == nil) then    
    pushKioskFrame(function()
      storyboard.gotoScene( "assets.exit.level1", "crossFade", 250 );
    end, 1000, 1000, "exiting_completion_s2");
...
function scene:didExitScene( event )
  display.remove(exit);
  exit=nil;
  storyboard.purgeScene( "assets.exit.exit1_2" );
