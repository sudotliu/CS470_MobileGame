...
-- Purpose: show the exit completion for level 2
-- Group 5:
--    Terry Liu
--    Raymond Ononiwu
--    William Thomas
-- Description: 
--    This will show after level 2. Click to continue
-----------------------------------------------------------------------------------------

-- This sets up the exit screen of the game -------------------------------------
print("Loading assets.exit.level2...");
...
local function onSceneTouch( self, event )
  if event.phase == "began" then
    ...
    storyboard.gotoScene( "assets.completion_screen", "crossFade", 250 );
...
function scene:createScene( event )
  ...
  exit = display.newImage("images/exit/level2exit.png", true);
...
function scene:willEnterScene( event )
  resetCamera();
  if(kiosk.labels["exiting_completion_l2"] == nil) then    
    pushKioskFrame(function()
      storyboard.gotoScene( "assets.completion_screen", "crossFade", 250 );
    end, 5000, 1000, "exiting_completion_l2");
...
function scene:didExitScene( event )
  display.remove(exit);
  exit=nil;
  storyboard.purgeScene( "assets.exit.level2" );
