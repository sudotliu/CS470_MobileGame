...
GeneratorValveII = class('GeneratorValveII') 

function GeneratorValveII:initialize(x, y, xF, yF, r, startUp, physics, valvegroup, watergroup, interval, maxP)
  self.r = r;
  self.particles = {};
  self.interval = interval;
  self.max = maxP;
  self.physics = physics;
  self.valvegroup = valvegroup;
  ...
  self.particleTimerID =   
    ...
    function() 
      ...
      elseif(self.count < self.max) then
        self:makeParticle(self.x, self.y, xF*(math.random()/2 + .5), 0);
      else
        self:stop();
      end      
    end, 0 );
...
function GeneratorValveII:setPos(x, y)
  self.x = x;
  self.y = y;
  -- Adding valve code for image
  self:updateImage();
...
function GeneratorValveII:updateImage()
  ...
  self.valvegroup:insert(self.image);
  self.addedImg=true;
...
function GeneratorValveII:switch()
...
function GeneratorValveII:makeParticle(x, y, xForce, yForce)
  local offset = self.image.width / 2 + 2;
  if(self.particles == nil) then return; end;
  -- First particle stream
  local particle = WaterParticle:new(x+offset,y-2,(math.random()*2.5)+1, self.physics,self.watergroup);
  ...
  local particle2 = WaterParticle:new(x+offset,y+2,(math.random()*2.5)+1, self.physics,self.watergroup);
  ...
  local particle3 = WaterParticle:new(x+offset,y+6,(math.random()*2.5)+1, self.physics,self.watergroup);
...
function GeneratorValveII:createOOBListener(particle)
...
function GeneratorValveII:createPUMPListener(particle)
  particle.image:addEventListener("collision",
  function(e)
...
function GeneratorValveII:start()
  self.on = true;
end

function GeneratorValveII:stop()
  self.on = false;
end

function GeneratorValveII:kill()
  self:stop();
