...
-- Used for smaller water particles
GeneratorValveSmall = class('GeneratorValveSmall') 

function GeneratorValveSmall:initialize(x, y, xF, yF, r, startUp, physics, group, interval, maxP)
  self.r = r;
  self.particles = {};
...
function GeneratorValveSmall:setPos(x, y)
  self.x = x;
  self.y = y;
  -- Adding valve code for image
  self:updateImage();
...
function GeneratorValveSmall:updateImage()
...
function GeneratorValveSmall:switch()
...
function GeneratorValveSmall:makeParticle(x, y, xForce, yForce)
  local offset = self.image.width / 2 + 2;
  if(self.particles == nil) then return; end;
  -- First particle stream
  local particle = WaterParticle:new(x+offset,y-2,(math.random()*1.5)+1, self.physics,self.group);
  ...
  local particle2 = WaterParticle:new(x+offset,y+2,(math.random()*1.5)+1, self.physics,self.group);
  ...
  local particle3 = WaterParticle:new(x+offset,y+6,(math.random()*1.5)+1, self.physics,self.group);
...
function GeneratorValveSmall:createOOBListener(particle)
...
function GeneratorValveSmall:createPUMPListener(particle)
  particle.image:addEventListener("collision",
  function(e)
    ...
    elseif (e.other.tag == 'pumpDown') then
      playSoundEffect(sounds.pump, -1, 3);
      audio.setVolume(0.9, {channel = 3});
      particle:useForce(0, 500);
    end
...
function GeneratorValveSmall:start()
  self.on = true;
end

function GeneratorValveSmall:stop()
  self.on = false;
end

function GeneratorValveSmall:kill()
  self:stop();
